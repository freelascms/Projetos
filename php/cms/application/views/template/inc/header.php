<!DOCTYPE html>
<!--[if lt IE 7]> <html class="no-js ie6 oldie" lang="pt-br" xmlns:og="http://ogp.me/ns#" xmlns:og="http://ogp.me/fb#"> <![endif]-->
<!--[if IE 7]> <html class="no-js ie7 oldie" lang="pt-br" xmlns:og="http://ogp.me/ns#" xmlns:og="http://ogp.me/fb#"> <![endif]-->
<!--[if IE 8]> <html class="no-js ie8 oldie" lang="pt-br" xmlns:og="http://ogp.me/ns#" xmlns:og="http://ogp.me/fb#"> <![endif]-->
<!--[if gt IE 8]> <!--> <html class="no-js" lang="pt-br" xmlns:og="http://ogp.me/ns#" xmlns:og="http://ogp.me/fb#"> <!--<![endif]-->
<head>
	<meta charset="UTF-8">
	<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
	<title>The Manager - Web Content Manager</title>
	<meta name="author" content="humans.txt"> 
	<meta name="description" content=" ">
	<meta name="keywords" content=" ">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<!-- meta tags default -->
	<meta property="og:title" content=" ">
	<meta property="og:type" content=" ">
	<meta property="og:url" content=" ">
	<meta property="og:image" content=" ">
	<meta property="og:site_name" content=" ">
	<meta property="og:description" content=" ">
	<!-- meta tags Open Graph protocol -->
	<link rel="shortcut icon" href="<?= base_url() . IMGLAYOUTPATH ?>favicon.ico">
	<link rel="icon" href="<?= base_url() . IMGLAYOUTPATH ?>favicon.ico">
	<link rel="apple-touch-icon" href=" ">
	<!-- favicon -->
	<link rel="stylesheet" href="<?= base_url() . CSSLAYOUTPATH ?>style.css">
	<!-- style -->
	<script src="<?= base_url() . JSLAYOUTPATH ?>modernizr.js"></script>
	<script src="<?= base_url() . JSLAYOUTPATH ?>jquery.js"></script>
	<script src="<?= base_url() . JSLAYOUTPATH ?>jquery-ui.js"></script>
	<!-- script -->
</head>
<body>
	<nav class="screen-reader">
		<a href="#content" accesskey="c">Alt + Shift + C ir para o conteúdo</a>
		<a href="#nav" accesskey="m">Alt + Shift + M ir para o menu</a>
		<a href="#search" accesskey="b">Alt + Shift + B ir para a busca</a>
		<a href="#footer" accesskey="f">Alt + Shift + F ir para o rodapé</a>
	</nav><!-- .screen-reader -->
	<div id="wrap">
		<div id="container">
			<header id="header">
				<h1><a href="index.php" title="The Manager - Web Content Manager">The Manager - Web Content Manager</a></h1>
				<h2>Projeto The Manager - CMS</h2>
				<nav id="nav">
					<ul>
						<li>Dereckson Santana
							<ul>
								<li><a href="#" title="Minha Conta">Minha Conta</a></li>
								<li><a href="#" title="Usuários">Usuários</a></li>
							</ul>
						</li>
						<li><a href="#" title="Sair">Sair</a></li>
					</ul>
				</nav>
			</header><!-- #header -->